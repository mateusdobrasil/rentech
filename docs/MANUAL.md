...
**Contextos especiais (listas dinâmicas).** Algumas automações precisam de uma lista calculada em código, injetada na mensagem via `{{lista}}`/`{{quantidade}}`, em vez de só `{{primeiro_nome}}`/`{{nome_completo}}`. Isso é resolvido pelo mapa `CONTEXTOS_ESPECIAIS` em `app/api/cron/motor/route.ts`, indexado pela `chave` da automação — se a função devolver `null` (nada a reportar), o motor pula essa automação sem disparar. Casos hoje: `frota-vencimentos` (`app/lib/frota.ts`, veículos com CRLV/seguro vencido), `documentos-vencidos` (`app/lib/documentos.ts`, documentos de funcionários/empresa vencidos) e `aniversariantes-da-semana` (`app/lib/aniversarios.ts`, funcionários ativos que fazem aniversário na semana — Seg a Dom — corrente; destinatários fixos, tipicamente o RH, configurados normalmente em "Selecionar funcionários específicos").
