...
// direto no bucket (ver uploadDocumentoAction abaixo pro porquê) — mantém a
// mesma convenção de sempre (funcionario/categoria/timestamp-nome.ext).
export async function storagePathDocumentoAction(payload: {
  funcionarioNome: string; categoriaId: number; nomeArquivo: string;
  ...
  const db = supabaseAdmin();
  try {
    const { data: cat } = await db.from('folha_documento_categorias').select('nome').eq('id', payload.categoriaId).maybeSingle();
    const catSlug = slug(cat?.nome || 'outros');
    const path = `${slug(payload.funcionarioNome)}/${catSlug}/${Date.now()}-${slug(payload.nomeArquivo.replace(/\.[^.]+$/, ''))}.${(payload.nomeArquivo.split('.').pop() || 'bin').toLowerCase()}`;
...
// no anexo de OPs), e um documento digitalizado um pouco maior já bastava
// pra disparar "An unexpected response was received from the server".
export async function uploadDocumentoAction(payload: {
  ...
  const db = supabaseAdmin();
  const { funcionarioNome, categoriaId, titulo, storagePath, nomeArquivo, tipoMime, tamanhoBytes, dataValidade, observacao, enviadoPor, visivelPortal } = payload;

  if (!funcionarioNome || !categoriaId || !storagePath) {
