...
interface ItemLote {
  ...
  origem: string | null; // 'FICHA' | 'OCR' (só adiantamento)
                          ...
                          return (
                            <tr key={chaveEdit} className={`${idx % 2 === 1 ? 'bg-[#F8FAFC]' : 'bg-white'} border-b border-[#E2E8F0] ${it.metodo === 'SEM_DADOS' ? 'opacity-60' : ''}`}>
                              ...
                              <td className="p-3">
                                ...
                                {it.origem && <span className="ml-1 text-[9px] font-bold text-gray-400 uppercase" title={it.origem === 'FICHA' ? 'Valor fixo da ficha' : 'Lido do holerite (OCR)'}>{it.origem === 'FICHA' ? '📋 ficha' : '🔍 ocr'}</span>}
                              </td>
                              <td className="p-3">
                                {it.metodo === 'SEM_DADOS'
