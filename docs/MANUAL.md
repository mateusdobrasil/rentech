...
- **Automações de Agendamentos e Disparos** (`dispararAutomacaoWhatsApp` em `app/lib/automacoes.ts`): ainda **não resolvido** — o texto é livre e editado pelo RH a qualquer momento, o que não casa diretamente com o modelo de template pré-aprovado da Meta. Enquanto isso não for implementado, evitar usar Meta no escopo de Envio para automações reais (Z-API não tem essa restrição).
