...
export default function ComercialHub() {
    ...
    const carregarAcesso = async () => {
      ...
      const [perfilRes, permissoesRes] = await Promise.all([
        ...
        supabase.from('folha_paginas_permissoes').select('endereco_route, permissoes_permitidas')
          .in('endereco_route', modulosComercial.map(m => m.link))
      ]);
  ...
  // não de um array fixo no código. Rota sem linha na tabela = ninguém acessa.
  // ==========================================================================
  const modulosAutorizados = modulosComercial.filter(modulo =>
