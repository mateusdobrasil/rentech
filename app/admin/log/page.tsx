...
export default function PainelAuditoria() {
  ...
  useEffect(() => {
    async function checkAuth() {
      ...
      const p = String(perfil?.permissao || '').toUpperCase();
      if (!p.includes('ADMIN') && !p.includes('DIR') && !p.includes('GEREN')) {
        router.push('/admin');
        return;
      }
  ...
  useEffect(() => {
    if (!authLoading) carregarLogs(true, []);
  }, [authLoading, filtroSetor, filtroData, buscaAplicada]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen bg-[#F0F4F8] flex items-center justify-center pt-16">
        <div className="w-10 h-10 border-4 border-[#E2E8F0] border-t-[#336699] rounded-full animate-spin" />
      </div>
    );
  }
