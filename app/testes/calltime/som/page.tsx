...
export default function CallTimeSom() {
  ...
  const [emTurno] = useState(() => turnoDoPosto('som') !== null);
  const [postoDoTurno] = useState(() => posicaoNoTurno('som'));
  const [estado, setEstado] = useState<EstadoSom>(() => {
    const turno = turnoDoPosto('som');
  ...
  const reiniciar = useCallback(() => {
    ...
    const turno = turnoDoPosto('som');
    setEstado(turno ? criarPartida(turno.obras.som) : criarPartida());
    setFase('os');
    setCamada('pa');
    setVista('planta');
    setFerramenta('caixa');
